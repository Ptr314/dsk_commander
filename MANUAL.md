...
Версия 2.x.x представляет собой двухпанельный файловый менеджер в стиле Norton Commander. Левая и правая панели являются равноценными и позволяют одновременно просматривать содержимое диска и хост-файловую систему, или два разных диска.
...
* **Экспорт образа в другой формат** (Ctrl-F2) &ndash; [конвертация открытого образа](#экспорт_диска) в другой формат. 

* **Просмотр** (F3) &ndash; для образов в хост-системе &ndash; вывод информации об образе, для файлов в образах &ndash; [просмотр файлов](#просмотр_файла) в различных форматах.

* **Информация о файле** (Ctrl-F3) &ndash; вывод информации файловой системы о файле внутри образа.

* **Редактирование метаданных** (F4) &ndash; [редактирование служебной информации](#редактирование_метаданных) о файле (возможности зависят от файловой системы).

* **Переименование файла или директории** (F7) &ndash; переименование файлов и директорий.

* **Восстановление файла или директории** (F9) &ndash; восстановление удалённых файлов или директорий, если это возможно. В данный момент доступно для дисков DOS33. **Полноценное восстановление возможно только в том случае, если сектора, ранее занятые файлом, не перезаписаны позднее другими файлами!**
...
В частности, если файлы копируются из образа DOS33 в хост-систему, будут доступны два формата &ndash; двоичный, когда файл будет сохранён &laquo;как есть&raquo;, и [FIL](https://agatcomp.ru/agat/PCutils/FileType/FIL.shtml) - помимо данных, сохарняется служебная информация.
...
При удалении файлов с компьютера (просмотре хост-системы) файлы перемещаются в корзину (Recycle Bin на Windows, Trash на macOS/Linux) вместо окончательного удаления. Это позволяет восстановить файлы при случайном удалении.
